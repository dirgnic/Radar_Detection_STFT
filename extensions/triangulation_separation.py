...
def maximum_filter_2d(image, size = 3):
    h, w = image.shape
...
def find_local_maxima(image, window_size = 3):
...
class DelaunayTriangulation:
    ...
    def add_point(self, point_idx):
        ...
        for tri in self.triangles:
            if self.in_circumcircle(point, tri):
                bad_triangles.append(tri)
        polygon = []
        for tri in bad_triangles:
            for i in range(3):
                edge = self.get_edge(tri, i)
                is_shared = False
        ...
        for edge in polygon:
            new_tri = Triangle((point_idx, edge[0], edge[1]))
            self.triangles.append(new_tri)

    def in_circumcircle(self, point, tri) -> bool:
        a = self.points[tri.vertices[0]]
        b = self.points[tri.vertices[1]]
        c = self.points[tri.vertices[2]]
        a = a - point
        b = b - point
        c = c - point
    ...
    def triangle_has_edge(self, tri, edge):
...
class TriangulationSeparator:
    ...
    def detect_peaks_manual(self, magnitude):
        threshold = np.percentile(magnitude, self.peak_threshold_percentile)
        local_max = find_local_maxima(magnitude, self.min_peak_distance)
    ...
    def group_triangles_manual(self):
        ...
        for sharing_triangles in edge_to_triangles.values():
            if len(sharing_triangles) == 2:
                ...
                if abs(mag_i - mag_j) / max(mag_i, mag_j) < self.connectivity_threshold:
        ...
        for i in range(n_triangles):
            if triangle_labels[i] != -1:
                continue
            stack = [i]
            triangle_labels[i] = component_id
            while stack:
                curr = stack.pop()
        ...
        for c_id in range(component_id):
            ...
            pt_indices = np.unique(simplices[comp_tri_idx])
            points = self.peaks[pt_indices]
            f_idx, t_idx = points[:, 0], points[:, 1]
            energy = np.sum(magnitude[f_idx, t_idx] ** 2)
    ...
    def separate_manual(self, stft_magnitude, freqs, times):
        print("\n[SEPARARE CU TRIANGULARE]")
        self.stft_magnitude = stft_magnitude
        self.freqs = freqs
        self.times = times
        print("Detectia varfurilor")
        self.detect_peaks_manual(stft_magnitude)
        print("Triangulare Delaunay")
        self.triangulate_manual()
        print("Gruparea componentelor")
        components = self.group_triangles_manual()
...
def visualize_triangulation(separator, output_path="./spectrograms/triangulation_separation.pdf"):
    ...
    sxx_db = 20 * np.log10(separator.stft_magnitude + 1e-6)
