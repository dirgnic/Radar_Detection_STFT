...
\begin{frame}{Adaptări pentru Sea Clutter Real}
\begin{columns}[T]
\column{0.5\textwidth}
\textbf{1. K-Distribution}:
\begin{itemize}
    \item Sea clutter $\neq$ Gaussian
    \item Pondere mai mare în cozi
...
\begin{itemize}
    ...
    \item Țintă perturbă: $H < 0.6$
    \item Detectează ținte slabe
\end{itemize}

\column{0.5\textwidth}
\textbf{3. DBSCAN Asimetric}:
\begin{itemize}
    \item Ținte = linii verticale în TF
    \item Scalare frecvență: $s_f = 3.0$
    \item Un cluster per țintă
\end{itemize}
...
\caption{\small GOCA-CFAR pe Target \#17}
\end{figure}

\column{0.5\textwidth}
\begin{figure}
\includegraphics[width=\textwidth]{ipix_target_17_fractal_frame83.png}
\caption{\small Cu Fractal Boost}
\end{figure}
\end{columns}

\vspace{0.2em}
\small IPIX: Radar X-band 9.39 GHz, PRF=1000 Hz, țintă sferă 1m la 2660m
\end{frame}
...
\begin{tabular}{lccc}
\toprule
\textbf{Metodă} & \textbf{Robustețe clutter} & \textbf{Ținte slabe} & \textbf{Ideal pentru} \\
\midrule
...
\vspace{0.5em}
\textbf{Observație experimentală}: CA-CFAR eșuează pe IPIX -- detectează clutter, nu ținte!

\vspace{0.3em}
GOCA-CFAR + DBSCAN = performanța optimă pentru sea clutter neomogen
\end{frame}

\begin{frame}{Comparație Spectrograme Metode}
\begin{figure}
\centering
\includegraphics[width=0.95\textwidth]{comparisons_paper_method_vs_other_methods.pdf}
\caption{\small Comparație spectrograme: Metoda CFAR-STFT vs. alte metode}
\end{figure}
\end{frame}
...
\begin{itemize}
    ...
    \item Triangulare: fragmentare excesivă, inconsistentă între scenarii
\end{itemize}
\end{frame}
