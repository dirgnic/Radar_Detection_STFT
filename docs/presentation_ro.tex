...
\textbf{Sea Clutter}:
\begin{itemize}
    \item Ecouri de la suprafața mării
    \item Non-Gaussian cu spike-uri frecvente
...
\textbf{Sea clutter $\neq$ Gaussian} -- modelează spike-uri cu K-distribution:
...
\vspace{0.3em}
\textbf{Ajustare prag}: $\nu$ mic $\Rightarrow$ mai spiky. Estimare $\nu = \mu^2/(\sigma^2 - \mu^2)$, ajustare multiplicator prag CFAR.
\end{frame}
...
\textbf{Alte adaptări}:
\begin{itemize}
    \item Mascare componentă DC: \(\pm\)8 binuri (\(\approx\)\(\pm\)31 Hz)
    \item Filtru bandwidth Doppler: BW \(\geq\) 2 binuri (\(\approx\)8 Hz)
\end{itemize}
\column{0.5\textwidth}
\end{columns}
\end{frame}
