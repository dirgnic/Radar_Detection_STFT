...
\subsubsection{src/cfar\_stft\_detector.py (1110 linii)}
...
\begin{itemize}
    \item \textbf{CFAR2D} (liniile 100-320): Implementare GOCA-CFAR 2D
        \begin{itemize}
            \item \texttt{detect()}: versiune nested-loop clasică
            \item \texttt{detect\_vectorized()}: versiune cu convoluție SciPy (rapid)
        \end{itemize}
    \item \textbf{DBSCAN} (liniile 350-500): Clustering custom în coordonate fizice (Hz, s)
        \begin{itemize}
            \item \texttt{fit()}: segmentare principală
            \item \texttt{\_region\_query()}: interogare rază optimizată
        \end{itemize}
    \item \textbf{CFARSTFTDetector} (liniile 550-950): Pipeline complet CFAR-STFT
        \begin{itemize}
            \item \texttt{compute\_stft()}: calcul STFT cu fereastră Gaussiană
            \item \texttt{detect\_components()}: pipeline complet detecție
            \item \texttt{reconstruct\_component()}: iSTFT cu mască
            \item \texttt{get\_doppler\_info()}: analiză Doppler
        \end{itemize}
    \item \textbf{AcousticCFARDetector}: Variantă pentru detecția acustică
\end{itemize}
