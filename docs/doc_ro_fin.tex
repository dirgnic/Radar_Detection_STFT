...
\end{tabular} &
Detectează candidații de target pe baza unui prag adaptiv local aplicat în 4 cadrane; controlează rata de alarmă falsă. \\ \midrule

DBSCAN -- clustering pe densitate &
\begin{tabular}[t]{@{}>{\raggedright\arraybackslash}p{\linewidth}@{}}
$d=\sqrt{(\Delta t)^2 + (\Delta f/s_f)^2}$\\
\textit{Anizotropic} $\Rightarrow$ scalare diferită pe axe; \textit{aici} $s_f=3$ (favorizează elemente verticale)
\end{tabular} &
...
decizie $|X(k,n)|^2 > T$.
\item \textbf{DBSCAN}:\\
distanță anizotropică (scalare diferită pe axe: $\Delta f/s_f$); tipic $\varepsilon=8$, \texttt{minSamples}=5.
...
\textbf{Motivul succesului:} (1) GOCA adaptează prag pe 4 cadrane local, (2) DBSCAN anisotropic ($\text{freq\_scale}=3.0$) păstrează coeziunea target-urilor (linii verticale), (3) K-distribution + Hurst + DC masking + filtrul Doppler reduce alarmele false generate de clutter.
...
Estimează parametrii distribuției K (de ex. prin MLE sau momente) pe regiuni fără target; ajustează factorul de prag $\alpha$ astfel încât $P_{fa}$ să fie respectat; aplică în GOCA-CFAR local (4 cadrane). \\ \midrule

Hartă Hurst (fractalitate) &
Separă target-uri slabe ($H<0.6$) de clutter ($H\approx 0.8$) &
Calculează $H$ pe patch-uri timp--frecvență; masca $M_H = [H < H_{th}]$; combină $M_H$ cu detecțiile CFAR pentru a evidenția target-urile slabe. \\ \midrule
