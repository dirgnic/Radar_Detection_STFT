...
$X(k,n) = \sum_{m=0}^{N-1} x(m) \, w(m - nH) \, e^{-j2\pi km/N}$, cu $N = 256$, $H = 32$ (87.5\% overlap), fereastră Gaussiană $w(m) = e^{-m^2/(2\sigma^2)}$, $\sigma = 8$ &
Transformă semnalul $x[n]$ în plan timp--frecvență; oferă reprezentarea $|X(k,n)|^2$ pe care se aplică ulterior CFAR 2D și clusteringul. \\ \midrule

CFAR 2D (GOCA-CFAR) &
Prag adaptiv local: $H(k,n) = 1$ dacă $|X(k,n)|^2 > T$, unde $T = R \, \hat{Z}$, $\hat{Z} = \max(\mu_1, \mu_2, \mu_3, \mu_4)$, $R = N_T (P_{fa}^{-1/N_T} - 1)$ &
Evaluează fiecare bin $(k,n)$ în context local (4 cadrane de training) și generează o mască binară $H(k,n)$ ce marchează candidații de detecție cu rată de alarmă falsă controlată. \\ \midrule

DBSCAN (clustering pe densitate) &
Metrică asimetrică pentru semnături verticale: $d = \sqrt{\Delta t^2 + (\Delta f / 3)^2}$ &
Gruparea punctelor detectate $(k,n)$ în clustere coerente (ținte) printr-un algoritm de tip densitate, sensibil la structura aproape verticală a componentelor în plan timp--frecvență. \\

\bottomrule
\end{tabular}
\caption{Fundamente teoretice: reprezentarea STFT, detecția CFAR 2D și clusteringul DBSCAN utilizate în algoritmul CFAR-STFT.}
\end{table}
...
\begin{tabular}{@{} l p{4.3cm} p{7.3cm} @{}}
\toprule
\textbf{Metodă} & \textbf{Model matematic} & \textbf{Pseudocod simplificat} \\ \midrule

K-distribution (model clutter) &
$p(x) = \frac{4}{\Gamma(\nu)} \left(\frac{\nu x^2}{2\mu}\right)^{(\nu+1)/2} K_{\nu-1}\left(\sqrt{\frac{2\nu x^2}{\mu}}\right)$; cozi mai grele decât distribuția Gaussiană; $\nu = \mu^2 / (\sigma^2 - \mu^2)$ &
\textbf{1.} Estimează media $\mu$ și varianța $\sigma^2$ din ferestre de sea clutter.\\
\textbf{2.} Calculează $\nu = \mu^2 / (\sigma^2 - \mu^2)$.\\
\textbf{3.} Ajustează factorul de scalare $R$ sau pragul CFAR pentru a controla $P_{fa}$ în cozi grele. \\ \midrule

Exponent Hurst (fractalitate) &
$\mathbb{E}\left[|X(t+\tau) - X(t)|^2\right] \propto \tau^{2H}$; clutter: $H \approx 0.75$--$0.85$, ținte: $H < 0.6$ &
\textbf{1.} Pentru fiecare patch timp--frecvență, estimează exponentul Hurst $H$.\\
\textbf{2.} Marchează \emph{Hurst anomaly} acolo unde $H < H_{th}$ și puterea locală este mare.\\
\textbf{3.} Combină masca: $M = M_{CFAR} \lor (M_{Hurst} \land M_{power})$ pentru a evidenția țintele slabe. \\ \midrule

DBSCAN anisotropic + măști Doppler &
$d = \sqrt{(\Delta t)^2 + \left(\frac{\Delta f}{3.0}\right)^2}$; mască DC $\pm 8$ bin-uri; lățime Doppler $\ge 3$ Hz &
\textbf{1.} Maschează bin-urile de frecvență din jurul DC ($\pm 8$) înainte de CFAR/clusterizare.\\
\textbf{2.} Rulează DBSCAN cu metrica anisotropică, parametrii $\varepsilon$ și \texttt{minSamples}.\\
\textbf{3.} Pentru fiecare cluster, calculează lățimea Doppler și respinge clusterele cu lățime $< 3$ Hz.\\
\textbf{4.} Păstrează drept ținte valide doar clusterele rămase. \\

\bottomrule
\end{tabular}
\caption{Adaptări ale algoritmului CFAR-STFT pentru sea clutter real: model statistic, criterii fractale și clustering anisotropic.}
\end{table}
