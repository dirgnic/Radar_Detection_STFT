...
\begin{center}
\LARGE \textbf{ANALIZA SEMNALELOR RADAR ÎN PREZENȚA ZGOMOTULUI MARIN (SEA CLUTTER)}
\end{center}
...
\begin{tikzpicture}[scale=0.85]
	    % Formula box: explicit 2 lines (avoid ugly wrapping in a narrow box)
	    \node[draw, rounded corners, fill=gray!10, text width=7.2cm, minimum width=7.2cm, minimum height=4cm, align=left] (formulas) {
	        ...
	        {\small $\hat{Z}=\max(\mu_1,\mu_2,\mu_3,\mu_4)$}\\
	        {\small $T=\alpha\cdot\hat{Z}$}\\[0.4em]
...
\textbf{4.} Reindexează etichetele (0, 1, 2...) \\ \midrule
	...
	Bazat pe densitate & 
	\begin{tabular}[t]{@{}l@{}}
	Grupează puncte cu distanță $\le \varepsilon$. \\
	(Coordonate normalizate la rezoluția STFT: $t/dt$, $f/df$.)\\
	Anizotropic: \\[0.25em]
	$d=\sqrt{(s_t\Delta t)^2 + (\Delta f/s_f)^2}$,\quad $s_f=3,\; s_t=1.5$
	\end{tabular} \\
	\bottomrule
	\end{tabular}
...
2. Pentru fiecare $p$: inserează; reface triangularea local \\
...
\textbf{Motivul reușitei:} (1) GOCA adaptează pragul local pe 4 cadrane de training (colțuri), (2) DBSCAN anizotropic ($s_f=3.0$, $s_t=1.5$) păstrează coeziunea target-urilor (linii verticale), (3) K-distribution + extensia Hurst reduc alarmele false generate de clutter.
...
Parametrii K se estimează o dată per rulare CFAR, dintr-un eșantion larg al hărții TF (excluzând vârfuri extreme); pragul se obține din ICDF pentru $P_{fa}$ și se aplică adaptiv local prin GOCA ($\hat{Z}=\max(\mu_i)$). \\ \midrule
