...
\begin{enumerate}
    ...
    \item analiză Doppler pentru estimarea vitezei radiale (din frecvența Doppler);
    \item documentație detaliată și reproductibilitate;
    \item validare rezultate: detecție și reconstrucție a componentelor;
    \item \textbf{adaptări sea clutter}: K-distribution, îmbunătățire bazată pe exponentul Hurst, DBSCAN asimetric.
\end{enumerate}
...
\begin{equation}
R = N_T \left( P_{fa}^{-1/N_T} - 1 \right)
\end{equation}
...
\begin{algorithmic}[1]
\Require Matrice STFT $X_{stft}$, $P_{fa}=0.001$, $N_G=3$, $N_T=12$
...
\For{$k \gets N_G + N_T$ to $N_f - N_G - N_T - 1$}
    \For{$n \gets N_G + N_T$ to $N_t - N_G - N_T - 1$}
        ...
        \State $\lambda \gets R(P_{fa}) \cdot \mathcal{N}_{local}$
...
Componenta fundamentală a proiectului este folosirea datelor reale din \textbf{IPIX} (Intelligent Pixel Processing for X-band radar), de la \textbf{McMaster University, Canada}. Datele provin de la un radar coerent polarimetric X-band folosit pentru monitorizarea activității maritime.
...
\textbf{Ce este clutter-ul marin?}

Clutter-ul marin (\textit{sea clutter}) reprezintă \textbf{ecouri radar de la suprafața mării}:
...
\caption{Explicație vizuală: ce este clutter-ul marin în datele IPIX? Figura arată spectrograma completă (two-sided), zoom pe regiunea clutter în jurul DC (frecvență 0 Hz) și distribuția energiei pe frecvențe. Notă: majoritatea energiei (>90\%) este concentrată în $\pm 50$ Hz în jurul DC.}
...
Algoritmul original presupune statistică Gaussian/Rayleigh, însă sea clutter real prezintă pondere mai mare în cozi (spike-uri frecvente) și urmează \textbf{K-distribution}:
...
\textbf{Soluție}: Estimare parametru din celulele de training: $\nu = \mu^2 / (\sigma^2 - \mu^2)$. Apoi ajustăm multiplicatorul pragului pentru a reduce alarmele false în clutter cu pondere mai mare în cozi.

\section{Adaptarea 2: Îmbunătățire bazată pe exponentul Hurst}

CFAR poate rata ținte slabe sub prag. Sea clutter are proprietate fractală (auto-similaritate) cuantificată de exponentul Hurst:
...
\textbf{Soluție}: Calculăm $H$ per bin de frecvență (analiză R/S). Dacă $H < 0.6$, marcăm punctele cu putere mare ca ținte potențiale. Combinăm cu CFAR: $\text{mască} = \text{CFAR} \lor (\text{anomalie Hurst} \land \text{putere mare})$. Această abordare îmbunătățește detecția țintelor slabe pe care CFAR singur le-ar rata.
...
\begin{enumerate}
    ...
    \item \textbf{Validare reală}: Funcționează pe date IPIX cu sea clutter complex
\end{enumerate}
