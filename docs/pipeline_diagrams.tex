...
\node[box, fill=cyan!20, right=of detector2] (doppler) {Analiză Doppler\\$v_r = \frac{f_d \cdot c}{2 f_{RF}}$};
...
% Arrow down from Cluster C1 to extended mask
\draw[-{Stealth[length=4mm]}, thick] (11.5,0) -- (11.5,-1);
\node[right] at (11.5,-0.4) {\scriptsize Geodesic};
\node[right] at (11.5,-0.65) {\scriptsize Dilation};
...
\end{tikzpicture}
\caption{Fluxul de procesare în planul timp-frecvență: spectrograma $\rightarrow$ detecții CFAR $\rightarrow$ clustering DBSCAN $\rightarrow$ extindere mască $\rightarrow$ mascare STFT $\rightarrow$ reconstrucție iSTFT.}
...
\textbf{Parametru} & \textbf{Paper} & \textbf{Nostru} & \textbf{Note} \\
\hline
Rata de eșantionare $f_s$ & 12.5 MSa/s & 12.5 MSa/s & Identic pentru chirp sintetic \\
Durata semnalului $T$ & 30 µs & 30 µs & Identic pentru chirp sintetic \\
Dimensiune FFT $N_{fft}$ & 512 & 512 & Identic \\
Fereastră & Gaussiană & Gaussiană & $\sigma = 8$ bins (identic) \\
Hop $H$ & N/A & 256 & 50\% overlap (standard) \\
$N_G$ (guard cells) & 16 & 16 & Identic \\
$N_T$ (training cells) & 16 & 16 & Identic \\
$P_f$ (prob. alarmă falsă) & 0.4 & 0.4 & Identic \\
Tip CFAR & GOCA & GOCA/CA & GOCA (exact) sau CA (vectorizat) \\
DBSCAN $\varepsilon$ & N/A & 3-5 Hz/s & Calibrat pentru semnale \\
DBSCAN min\_samples & N/A & 3-5 & Calibrat pentru zgomot \\
Simulări Monte Carlo & 100 & 100 & Identic \\
\textbf{Extra features} & -- & Vectorizare & Opțiune pentru viteză \\
\end{tabular}
};
\end{tikzpicture}
\caption{Comparație între parametrii din paper (Abratkiewicz 2022) și implementarea curentă. Parametrii principali sunt identici; adăugări: vectorizare CFAR opțională, calibrare DBSCAN pentru diverse semnale.}
\label{fig:params}
\end{figure}
